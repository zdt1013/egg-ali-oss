...
describe('test/oss.test.js', () => {
  ...
  describe('oss', () => {
    ...
    after(async () => {
      if (lastUploadFileName) {
        await app.aliOss.delete(lastUploadFileName);
      }
      await app.close();
    });

    it('should app.aliOss put file ok', async () => {
      const result = await app.aliOss.put(path.basename(__filename), __filename);
      assert(result.url);
      assert(result.res.status === 200);
    });

    it('should return promise', async () => {
      const p = app.aliOss.put(path.basename(__filename), __filename);
    ...
    it('should pass httpclient', async () => {
      assert(app.aliOss.urllib === app.httpclient);
    });
  });
  ...
  describe('oss with clients', () => {
    ...
    after(async () => {
      if (lastUploadFileName) {
        await app.aliOss.get('oss2').delete(lastUploadFileName);
      }
      await app.close();
    });
  ...
  describe('endpoint with http', () => {
    ...
    it('should set http', async () => {
      assert(app.aliOss.options.endpoint.hostname === 'oss.aliyun.com');
      assert(app.aliOss.options.endpoint.protocol === 'http:');
    });
  });
  ...
  describe('endpoint with https', () => {
    ...
    it('should set https', async () => {
      assert(app.aliOss.options.endpoint.hostname === 'oss.aliyun.com');
      assert(app.aliOss.options.endpoint.protocol === 'https:');
    });
  });
